...
5. **Stay inside your file ownership, and leave VCS alone.** Touch only the paths your packet assigns; other paths may belong to workers running concurrently. Do not commit, push, or otherwise change VCS state — snapshots and merges are the supervisor's job. To set changes aside for a moment — e.g. to confirm a new test fails without your fix — use `agent-exec shelf push [-- PATH...]` and `agent-exec shelf pop`, never `git stash`: the stash stack is shared with every other worktree, and parallel workers popping it have swapped each other's changes.
