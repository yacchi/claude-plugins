...
6. **Reclaim the worktrees before you report.** Once the last gate has closed and you have taken every diff out (`agent-exec isolate diff`/`collect`), run `agent-exec isolate sweep --include-current --text` once. This is the primary cleanup, not a courtesy: the `SessionEnd` hook never fires for a session that is killed or crashes, and when it does fire it is racing a teardown timeout — which is how repos accumulate orphaned orchestra worktrees. It cannot destroy anything unread: the sweep applies the same review gate as `isolate remove`, so a tree still holding uncollected work survives and is listed. If it reports any `dirty` tree, say so in your verdict (task id and path only, never its contents) so the instructor knows work is still sitting there.
7. **Report a structured verdict, nothing else.** Your final response to the instructor must be a compact structured verdict plus any exceptions, following the same shape orchestra-review uses, extended with round count. Never include code, diffs, logs, or intermediate artifacts.
