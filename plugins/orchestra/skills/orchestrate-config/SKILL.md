...
description: Interactively detect Codex/Copilot CLI and agent availability, then configure orchestra's model tiers and external-executor settings, writing YAML at project (`.claude/orchestra.yaml`) or user (`~/.claude/orchestra.yaml`) scope with correct deep-merge override semantics. Invoke with /orchestrate-config, or whenever the user wants to enable/disable Codex or Copilot dispatch, change model tiers, set a project-specific override, or migrate an old orchestra.json.
when_to_use: Use when the user asks to configure orchestra, turn Codex or Copilot on/off, add a per-project override on top of their personal orchestra config, or convert a legacy orchestra.json to the current YAML format. Do not use this skill to actually run a task through the pipeline — that's the `orchestrate` skill; this one only edits its config file.
---

# orchestrate-config: configuration skill

This skill is a small companion to `orchestrate` (see its SKILL.md §9 for the config schema this file edits). It exists to make setting up `.claude/orchestra.yaml` / `~/.claude/orchestra.yaml` interactive and safe — detect what's actually available, ask where to write the change, and edit without clobbering the user's existing comments. It does not relitigate the schema itself, and it does not run the orchestration pipeline.
...
- Legacy `.claude/orchestra.json` / `~/.claude/orchestra.json` (the pre-YAML format) are no longer read by `orchestrate`. See step 4 for what to do when one is found.
...
2. Show the user the equivalent YAML (same keys, structure carried over 1:1 — use `examples/orchestra.yaml` from the `orchestrate` skill as the commented template to fill in).
...
4. Apply the change:
   - Target file doesn't exist yet: create it from `examples/orchestra.yaml` (in the `orchestrate` skill), but **trim it down to only the keys actually being set** — see the "smallest diff" rule in step 2. Keep a short comment on any field whose meaning isn't obvious from the key name alone.
...
- It does not change the model-tier defaults documented in `orchestrate` SKILL.md §3 — those are hard-coded fallback values; this skill only edits the override file that sits on top of them.
