...
- **`dispatch: "cli"`** — the executor is a non-interactive CLI (Copilot and opencode today). The instructor prepares a token with `agent-exec dispatch prepare --class <cls> --prompt-file F --workdir W [--run-id R] [--isolate ...] [--task ID]` (every routing/accounting flag is fixed here, at prepare time); `dispatchClass()` (§5) gives only that token to one relay agent, which runs `agent-exec dispatch --token ... --capture` (plus `--exhausted` if needed) and hands back its JSON verbatim. The token form accepts only `--capture`/`--exhausted`; combining it with `--run-id`, `--class`, `--workdir` or any other prepare-time flag exits 2 with `--token conflicts with <flag>`. `dispatch` calls `route` internally, so the `ready.<executor>.ok` gate has already been applied before the CLI ever runs.
