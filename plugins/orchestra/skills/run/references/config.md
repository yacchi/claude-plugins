...
Each session writes to one file, `<ledger-dir>/<session-id>.jsonl`, where the session id comes only from the `CLAUDE_CODE_SESSION_ID` environment variable and is used only as the filename. No session id means no write, so agent-exec outside Claude Code never creates a ledger. A supplied `--run-id <id>` adds `run: <id>` to the line; it must match `^[A-Za-z0-9_-]{1,64}$` and still exits 2 when invalid. Ledger lines retain the existing allowlist — `ts`, `executor`, `cls`, `model`, `status`, the integer usage counters, and delegated Codex `corr` — and also allow `run` as a string matching that expression; appends are best-effort and never alter dispatch behavior.

`agent-exec usage --run <id>` reads both tagged lines in any ledger file and every line in the legacy v0.18.0 `<id>.jsonl` file, de-duplicating overlap. `--session <id>` reads every line in `<id>.jsonl`, including tagged lines. Under session scope, Copilot and Codex report normal ledger totals when lines exist; delegated Codex lines resolve through the correlation id and retain the `delegated`/`measured` counters. Without matching lines they report `attributable: false` with reason `no matching run ledger data`; `--since` behavior is unchanged.

The ledger CLI is **`agent-exec ledger show [--session <id>] [--run <id>] [--json|--text]`**, **`agent-exec ledger archive [--json|--text]`**, and **`agent-exec ledger clear [--yes] [--json|--text]`**. `show` reports ledger contents as counts and summed integer fields per executor, and without a selector summarizes every ledger file. `archive` and `clear` use the telemetry commands' confirmation behavior against the ledger directory. Missing files or directories are empty, not errors; unknown options or malformed selectors exit 2, and an empty ledger exits 0.

After a successful append, at most once per process, files directly in the ledger directory whose mtime is older than `retention_days` days are deleted. Cleanup is best-effort and only targets `*.jsonl`; failures never affect dispatch, and nothing else is deleted.
