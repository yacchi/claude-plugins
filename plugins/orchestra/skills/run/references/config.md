...
- **`agent-exec usage [--since W] [--json|--text] [--source a,b] [--all-projects]`** — read-only aggregation of Claude transcripts, Codex rollouts, and Copilot telemetry records. Claude usage defaults to the current project; Copilot usage is available only when telemetry is enabled.
