...
Ledger lines contain only `ts` (ISO8601 UTC), `executor` (`copilot`|`codex`|`claude`), `cls` (`light`|`standard`|`deep`), `model`, `status` (`ok`|`error`|`unavailable`), and the non-negative integer counters `input_tokens`, `output_tokens`, `cached_input_tokens`, `aiu_nano`, `premium_requests`, `api_duration_ms`, and `session_duration_ms`; unavailable values are omitted rather than written as null, and booleans are rejected as counters. `model` must match `^[A-Za-z0-9._/-]{1,64}$`, and no other keys or free-form data are permitted. Ledger appends are best-effort and never alter dispatch behavior.

Codex rollout-file correlation and main-thread line-offset attribution are deliberately not implemented. Under `--run`, a source with no ledger entries therefore reports `attributable: false` rather than a guessed number, which keeps the output trustworthy.
