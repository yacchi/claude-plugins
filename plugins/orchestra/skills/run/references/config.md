...
Load this file only when a configuration question actually arises: what a key means, how the four layers merge, why `route` dropped a candidate, what `enforcement.light_class` does, or what telemetry records. For normal orchestration you do not need it — one `agent-exec doctor --json` call already returns the resolved config, and `agent-exec route`/`dispatch` already execute the priority walk.
...
The `--run-id <id>` flag is accepted by `agent-exec dispatch --class <cls>` and `agent-exec run <profile>`. The id must match `^[A-Za-z0-9_-]{1,64}$`; otherwise the command exits 2 and names the offending value. With it, exactly one sanitized JSON line is appended per dispatch to `os.path.join(os.path.dirname(_telemetry_dir_from_cfg(cfg)), "runs")/<id>.jsonl`, creating that directory with mode `0o700` on first write. The ledger is a separate, local-only store from telemetry and is never uploaded; writing it does not depend on `telemetry.enabled`, because passing `--run-id` is the consent.

Ledger lines contain only `ts` (ISO8601 UTC), `executor` (`copilot`|`codex`|`claude`), `cls` (`light`|`standard`|`deep`), `model`, `status` (`ok`|`error`|`unavailable`|`delegated`), and the non-negative integer counters `input_tokens`, `output_tokens`, `cached_input_tokens`, `cache_write_input_tokens`, `reasoning_output_tokens`, `aiu_nano`, `premium_requests`, `api_duration_ms`, and `session_duration_ms`; unavailable values are omitted rather than written as null, and booleans are rejected as counters. Delegated Codex lines additionally use `corr`, a string matching `^oxc-[0-9a-f]{12}$`. `model` must match `^[A-Za-z0-9._/-]{1,64}$`, and no other keys or free-form data are permitted. Ledger appends are best-effort and never alter dispatch behavior.

For `agent-exec dispatch --class <cls>`, when the resolved route is not `dispatch: cli`, the executor is not `claude`, and `--run-id` was passed, the delegate JSON contains a `correlation_id` matching `^oxc-[0-9a-f]{12}$` and one ledger line has `status: delegated` plus the corresponding `corr`; no token fields are written. For Claude, or when `--run-id` was not passed, `correlation_id` is absent, no id is minted, and no delegated ledger line is written.

Under `usage --run <run-id>`, each delegated Codex line is resolved by scanning Codex rollout files for a recorded task `user_message` containing its `corr`. The matching rollout's last cumulative `total_token_usage` is used, mapping `input_tokens`, `output_tokens`, `cached_input_tokens`, `cache_write_input_tokens`, and `reasoning_output_tokens`; `total_tokens` is deliberately not used because it is a sum of the other fields. If multiple rollouts match, use the one with the most `token_count` lines, breaking ties by lexicographically greatest path, and count it once.

The Codex entry in a run report includes `delegated` and `measured`. If `measured == 0` and `delegated > 0`, it is `{"attributable": false, "reason": "codex delegated but no rollout matched", "delegated": N, "measured": 0}`; if `measured > 0`, it reports `status: ok` with summed tokens and both counters, even when `measured < delegated`. In `--text`, these render as `codex   [ok]  in=... out=... cached=...  (measured 2/3 delegated)` or `codex: not attributable (codex delegated but no rollout matched) (measured 0/3 delegated)`. A dispatch whose rollout cannot be found is reported as having run but not been measured, never as a guessed number. The `--since` window scope keeps its existing Codex behavior exactly and does not use this correlation.

Codex has no CLI invocation path; it stays on `dispatch: agent` and its official `codex:codex-rescue` agent. There is no change to `dispatch: agent`, no time-window correlation of rollouts, and no agent-run "close the ledger" command.
