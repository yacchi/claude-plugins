...
external_executors:
  ...
  copilot:
    enabled: false
    dispatch: cli
    # no --allow-all-tools: blocked by Claude Code's Bash safety classifier even
    # when allowlisted — the grant comes from COPILOT_ALLOW_ALL=true in
    # settings.json `env` instead (see the dispatch: "cli" notes below)
    command: >-
...
- **`dispatch: "cli"`** — the executor is a non-interactive CLI. Write the task prompt to a temp file, substitute `{promptfile}` (and, if the config declares them, `{model}`/`{effort}`/`{workdir}`/`{session_id}`) in `command`, and have a **cheap relay agent** (`model: haiku`, with Bash) run the command and return only its final output. The instructor must never run the CLI via Bash itself — the relay exists to keep CLI stdout/stderr out of the instructor's context. **Because the relay runs non-interactively, the CLI command must already be allowlisted in Claude Code's Bash permissions** — an un-allowlisted call is denied outright (a subagent can't surface an approval prompt at call time), so the dispatch fails before the CLI ever runs. For Copilot this means `Bash(copilot:*)` must be in `permissions.allow` (project `.claude/settings.json` or user `~/.claude/settings.json`, matching the scope where you enabled Copilot); add it with `/permissions` or the `update-config` skill. **The allow rule alone is not sufficient: the command template must also be free of permission-bypass flags.** Claude Code's Bash safety classifier runs *on top of* the allowlist and blocks commands carrying flags like `--allow-all-tools` even when the top-level command matches an allow rule — which is why the shipped Copilot templates omit that flag and rely on Copilot's official equivalent env var instead: `COPILOT_ALLOW_ALL=true`, set in the `env` block of the same settings.json (never as an inline `COPILOT_ALLOW_ALL=true copilot ...` prefix, which both breaks the `Bash(copilot:*)` prefix match and re-exposes the grant to the classifier). Codex's `dispatch: "agent"` needs none of this, since it routes through a subagent rather than a raw Bash command. Treat that authorization as a **pre-flight availability check**: before including a `dispatch: cli` executor in a `priority` candidate list, the instructor confirms both the permission rule and (for Copilot) the `COPILOT_ALLOW_ALL` env setting are present in the session's effective settings and, if either is absent, drops it to a later candidate (or out) for this run — an unauthorized cli executor can never succeed here, so discovering it through a failed dispatch merely wastes a relay round (the relay's `STATUS: unavailable` stays as the runtime backstop). This authorization check is a cheap local settings read and is explicitly exempt from the "no proactive quota probing" rule (quota is transient and remote; authorization is static and local) — see `references/external-executors.md` §4.
