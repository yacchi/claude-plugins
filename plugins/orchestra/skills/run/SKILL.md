...
**Follow-up rounds:** when a subagent *you spawned with the Agent tool* needs another pass after it has returned, resume that same instance with `SendMessage` rather than spawning a fresh one — it still holds what already failed and what it has tried, so nothing needs re-explaining. **Never `SendMessage` an agent that a running Workflow spawned**, nor one that has not returned yet: observed, a message to a running workflow engineer resumed a *copy* of it, and the copy and the original committed into the same worktree seconds apart. Corrections inside a Workflow go through its own gates (§11); the `worktree_lease` guard denies a second writer in a worktree if it happens anyway.
...
When the request is dozens of session-sized packages with dependencies between them, §5's `pipeline(tasks, runTask)` is the wrong outer loop: packages need each other's *code*, HEAD moves under every package while it runs, and the plan itself must be produced by agents first. `references/programme.md` holds the shapes, all proven in one real run and all reusing §5's `runTask` unchanged per package:

- **Planning workflows**, read-only and written to a specs directory: audit → per-auditor adversarial verification (default verdict `refuted`) → completeness critic → Opus synthesis into packages; design → critique → revise, or two opposing designs → judge; reconcile → cross-check; and a reality check of every spec against the current code **immediately** before scheduling, since `files_owned` is a prediction.
- **A dependency scheduler**: a package starts when its `depends_on` are integrated and no package with overlapping `files_owned` is in flight; a failed dependency blocks its dependents transitively.
- **A rolling integration base**: one integration worktree per programme; every package's token is minted with `--workdir` pointing at it, so each package starts from everything integrated so far, and a serial Sonnet integrator replays it back with `agent-exec isolate integrate`, resolving conflicts, verifying, and reverting only the package that broke.
- **Carry-over instead of escalation**: work a package cannot do because another package owns the files goes to a carry file, and a periodic triage moves it into the not-yet-started spec that owns those files.
- **Budget first**: chunk the programme (`maxPackages`, default 12; `maxInFlight`, default 4), return the pending set, and show `agent-exec usage --run` between chunks. The run these shapes come from pinned a model on one script of 13, used 115.9M tokens, and exhausted the user's weekly quota with six days left.
