...
external_executors:
  ...
  copilot:
    enabled: false
    dispatch: cli
    # dispatched via the agent-exec wrapper (recommended): it injects
    # COPILOT_ALLOW_ALL internally, so the only permission rule needed is
    # Bash(agent-exec:*) — no --allow-all-tools, no settings.json env block
    # (see the dispatch: "cli" notes below)
    command: >-
...
- **`dispatch: "cli"`** — the executor is a non-interactive CLI. Write the task prompt to a temp file, substitute `{promptfile}` (and, if the config declares them, `{model}`/`{effort}`/`{workdir}`/`{session_id}`) in `command`, and have a **cheap relay agent** (`model: haiku`, with Bash) run the command and return only its final output. The instructor must never run the CLI via Bash itself — the relay exists to keep CLI stdout/stderr out of the instructor's context. **Because the relay runs non-interactively, the CLI command must already be allowlisted in Claude Code's Bash permissions** — an un-allowlisted call is denied outright (a subagent can't surface an approval prompt at call time), so the dispatch fails before the CLI ever runs. **Recommended setup for Copilot: install the `agent-exec` wrapper** (via the `setup` skill, or `agent-exec install` directly) — it injects `COPILOT_ALLOW_ALL` internally, so the only permission rule needed is a single `Bash(agent-exec:*)` in `permissions.allow` (project `.claude/settings.json` or user `~/.claude/settings.json`, matching the scope where you enabled Copilot), and the shipped command templates dispatch through it (`agent-exec copilot ...`, see the example config above). **Fallback (manual, no wrapper):** `Bash(copilot:*)` in `permissions.allow` AND `COPILOT_ALLOW_ALL=true` in the settings `env` block — Claude Code's Bash safety classifier blocks permission-bypass flags like `--allow-all-tools` even on allowlisted commands, which is why the manual path relies on Copilot's official env var equivalent instead (never an inline `COPILOT_ALLOW_ALL=true copilot ...` prefix, which both breaks the `Bash(copilot:*)` prefix match and re-exposes the grant to the classifier). Codex's `dispatch: "agent"` needs none of this, since it routes through a subagent rather than a raw Bash command. Treat that authorization as a **pre-flight availability check**: before including a `dispatch: cli` executor in a `priority` candidate list, the instructor confirms the required permission rule (`Bash(agent-exec:*)`, or for the manual fallback both `Bash(copilot:*)` and the `COPILOT_ALLOW_ALL` env setting) is present in the session's effective settings and, if not, drops it to a later candidate (or out) for this run — an unauthorized cli executor can never succeed here, so discovering it through a failed dispatch merely wastes a relay round (the relay's `STATUS: unavailable` stays as the runtime backstop). This authorization check is a cheap local settings read and is explicitly exempt from the "no proactive quota probing" rule (quota is transient and remote; authorization is static and local) — see `references/external-executors.md` §4.
