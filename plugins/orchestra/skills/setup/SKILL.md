...
- **Copilot** — `command -v copilot`. This one *is* invoked directly via `dispatch: cli`, so its presence is the real functional gate: enabling `external_executors.copilot` without the CLI installed will fail loudly the first time it's dispatched. Its `dispatch: cli` also requires a `Bash(copilot:*)` rule in Claude Code's `permissions.allow` (see step 5) — so when Copilot is found, also read the effective Claude Code permissions and report whether that rule is already present.
...
4. Apply the change:
   ...
   - **Enabling Copilot specifically**: `external_executors.copilot` uses `dispatch: cli`, so beyond the orchestra.yaml change it also needs `Bash(copilot:*)` in Claude Code's `permissions.allow` — without it the Haiku relay's `copilot` call is auto-denied (a non-interactive subagent can't be granted approval at call time; see `run` SKILL.md §9 and `references/external-executors.md` §2). Check whether the rule is already present in the effective permissions; if not, offer to add it via the `update-config` skill (or point the user to `/permissions`), at the settings scope matching where Copilot was enabled (project `.claude/settings.json` for a project-scope enable, user `~/.claude/settings.json` for a user-scope one). This is a separate file from orchestra.yaml — never assume enabling Copilot in the YAML is sufficient on its own.
