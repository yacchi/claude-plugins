...
- **Copilot** — `command -v copilot`. This one *is* invoked directly via `dispatch: cli`, so its presence is the real functional gate: enabling `external_executors.copilot` without the CLI installed will fail loudly the first time it's dispatched. Its `dispatch: cli` needs only a single authorization path: installing the `agent-exec` wrapper, which needs only a single `Bash(agent-exec:*)` rule in `permissions.allow`. Measurement on Copilot CLI 1.0.74 established that `copilot -p` runs file/shell/network tool calls autonomously in non-interactive mode without any allow-all flag or env var — `COPILOT_ALLOW_ALL` is no longer required for headless dispatch, and this skill no longer offers it as a setup path. **Rather than running the shim/uv/config/executor/permission checks one by one, call `agent-exec doctor`** (or, if the shim isn't installed yet, invoke doctor via the bootstrap path directly — see step 5 below) — one call returns the consolidated report: whether the shim is installed and on `PATH` (and, if so, whether it's anchored to the stable marketplace-clone path or the version-stamped cache path), whether `uv` is present, the merged config layers, per-executor `enabled`/`available`, whether `Bash(agent-exec:*)` was found in scanned settings files (best-effort — this scan does not see enterprise policy or CLI `--allowedTools`, so a `false` here is not conclusive proof the rule is absent), and a per-executor `ready.<name>.ok` verdict with a `missing[]` list of the specific unmet prerequisites. Report doctor's findings plainly instead of re-deriving them from separate reads.
