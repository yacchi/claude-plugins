...
- **Copilot** — `command -v copilot`. This one *is* invoked directly via `dispatch: cli`, so its presence is the real functional gate: enabling `external_executors.copilot` without the CLI installed will fail loudly the first time it's dispatched. Its `dispatch: cli` needs an authorization path (see step 5): the **recommended** one is installing the `agent-exec` wrapper, which needs only a single `Bash(agent-exec:*)` rule in `permissions.allow` (the wrapper injects `COPILOT_ALLOW_ALL` itself); the **alternative manual** path is a `Bash(copilot:*)` rule in `permissions.allow` AND `"COPILOT_ALLOW_ALL": "true"` in the settings `env` block. When Copilot is found, also check whether `agent-exec` is already installed (`command -v agent-exec`) and read the effective Claude Code settings to report which of the two authorization paths (if either) is already in place.
...
4. Apply the change:
   ...
   - **Enabling Copilot specifically**: `external_executors.copilot` uses `dispatch: cli`, so beyond the orchestra.yaml change it also needs an authorization path set up. **Recommended: install the `agent-exec` wrapper.** It injects Copilot's allow-all env internally, so the only Claude Code permission rule the user needs becomes a single `Bash(agent-exec:*)`. Have the user install it by running, in their own shell via a `!`-prefixed command, the bootstrap script at its **stable marketplace-clone path** with the `install` subcommand:
     ```
     ! sh ~/.claude/plugins/marketplaces/<MP>/plugins/orchestra/tools/agent-exec install
     ```
     Resolve `<MP>` to the actual marketplace directory name for this install — detect it by finding this plugin's stable clone path (`~/.claude/plugins/marketplaces/*/plugins/orchestra/`), not the version-stamped cache path (`~/.claude/plugins/cache/<MP>/orchestra/<version>/...`), which changes on every plugin update and would leave a stale shim. The interactive installer asks for a bindir (default `~/.local/bin/agent-exec`) and, once installed, prints the single required permission rule `Bash(agent-exec:*)` for the user to add. This **replaces** the previous guidance of adding `"COPILOT_ALLOW_ALL": "true"` to settings `env` plus a `Bash(copilot:*)` rule — check whether `Bash(agent-exec:*)` is already present in the effective settings; if not, offer to add it via the `update-config` skill (or point the user to `/permissions`), at the settings scope matching where Copilot was enabled (project `.claude/settings.json` for a project-scope enable, user `~/.claude/settings.json` for a user-scope one).
     **Alternative (manual, no wrapper):** for a user who chooses not to install `agent-exec`, the old two-part setup still works — `Bash(copilot:*)` in `permissions.allow` (without it the Haiku relay's `copilot` call is auto-denied: a non-interactive subagent can't be granted approval at call time) AND `"COPILOT_ALLOW_ALL": "true"` in the settings `env` block (Copilot's official env equivalent of `--allow-all-tools`; the flag itself must never appear in the command template because Claude Code's Bash safety classifier blocks permission-bypass flags even on allowlisted commands — see `run` SKILL.md §9 and `references/external-executors.md` §2). Only offer this path if the user declines the wrapper.
     Note settings `env`/`permissions.allow` changes take effect from the next session start. This is a separate file from orchestra.yaml — never assume enabling Copilot in the YAML is sufficient on its own.
...
- It does not verify that Codex or Copilot actually work end-to-end (auth, quota, etc.) — for the Codex CLI itself, that's the separately-installed `codex:setup` skill's job. Its primary scope is orchestra's own config file; the one adjacent thing it touches is offering to install the `agent-exec` wrapper (or, as a fallback, the manual `Bash(copilot:*)` permission rule and `COPILOT_ALLOW_ALL` env entry) to Claude Code settings when you enable Copilot (step 5), since that dispatch can't work without them.
