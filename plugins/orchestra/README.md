...
## Worktree isolation (v0.16.0)

Seven weeks of transcripts contain **35 destructive VCS commands run by workers** against the user's shared working tree — `git checkout -- <paths>`, `git restore`, `git reset --hard`, `git clean -fd` — spread across haiku, sonnet, and opus workers alike. The countermeasure had been prose, escalated three generations deep in the worker prompts ("Never run git checkout…" → "The ONE exception…" → "THE WORKING TREE IS ALREADY DIRTY AND THAT IS EXPECTED AND CORRECT"), and the accidents continued after each escalation. A worker's only view of the world is `git status`; a diff it did not author reads as contamination whatever its prompt says.

So as of v0.16.0 the fix is structural. `agent-exec dispatch` isolates by default (`--isolate auto`) whenever the tree holds uncommitted work, giving the worker a tree that contains its own work and nothing else — the mistake becomes a no-op. This covers external CLI executors, which the Workflow tool's own `isolation: 'worktree'` option cannot reach even though that is where most implementation work runs.

`agent-exec isolate create` carries the user's uncommitted work into the worktree and commits it as a baseline (so `isolate diff` shows only the worker's changes), copies gitignored dependency directories by copy-on-write clone (~2s for 83MB/9.5k files, versus every worker re-running its own install), and reuses one worktree per `--task` across retry rounds. Where [git-worktree-runner](https://github.com/coderabbitai/git-worktree-runner) is installed it goes through `gtr`, so the user's own `gtr.copy.include` patterns and `gtr.hook.postCreate` steps apply — and orchestra never *writes* gtr config: what it needs is injected for one subprocess via `GIT_CONFIG_COUNT`/`KEY_n`/`VALUE_n`, leaving `.git/config`, `~/.gitconfig`, and the committable repo-root `.gtrconfig` untouched.

A second layer, `hooks/guard-worker-vcs.sh` (`enforcement.worker_vcs`, on by default), denies destructive VCS commands from *subagents* against any non-`orchestra/*` branch. The main thread is never affected, and inside its own worktree a worker may reset freely. A third layer, `hooks/guard-worker-tree.sh` (`enforcement.worker_tree`, on by default), closes the same gap for plain writes: a worker that *has* an orchestra worktree but edits the user's main tree anyway. That is not hypothetical — a worker whose prompt opened with "WORKING TREE: `<path>` / cd there FIRST … Never touch files outside that path" ignored the line and ran Edit/Write against the same files in the user's tree, and its rejected output would have been what the user kept. Claude Code's own `sandbox.enabled` cannot cover this (session-global, and Bash-only). A fourth layer, `hooks/cleanup-worktrees.sh` (`enforcement.session_cleanup`, on by default), removes the finishing session's worktrees at `SessionEnd` — but only the ones with no uncollected changes, since a dirty worktree is the only record of what a worker tried. That hook is session-scoped by construction, so leftovers still accumulate — a killed session, a run from before the hook shipped, a legacy sessionless `orchestra/<task>` worktree, `session_cleanup: "off"`, or a tree that was dirty at `SessionEnd` and kept on purpose. `agent-exec isolate sweep` (v0.30.0) is the after-the-fact counterpart: repo-wide rather than session-scoped, running `git worktree prune` first, applying the *same* review gate as `isolate remove` so uncollected work is reported and kept, skipping the running session's own trees unless `--include-current`, filtering by branch-tip age with `--older-than`, and deleting `orchestra/*` branches that outlived their worktree only on an explicit `--branches`. `--dry-run` previews through that identical gate, so the preview and the sweep can never disagree. The `/cleanup` skill is its user-facing front end.

The same release moves the *primary* cleanup off `SessionEnd` entirely, because that hook is unreliable in both directions: it never fires for a session that is killed, crashes, or has its terminal closed, and when it does fire it is racing a teardown timeout that a slow removal loses. The `run` skill's Workflow template therefore sweeps at **run end** — the moment the last gate closes, every diff is collected, and nothing is racing it — via the same cheap haiku relay that emits telemetry; `orchestra-delegate` does the same before reporting on the no-Workflow path. `SessionEnd` and `/cleanup` become backstops rather than the mechanism. Two changes make that safe and fast: the removal gate now skips `git diff --binary` whenever the patch text cannot change its verdict (a worktree nobody collected costs one `--name-only` instead of a full binary diff, which is what made bulk cleanup outrun its timeout), and parallel sessions are protected by a heartbeat — every `agent-exec isolate`/`dispatch`/`route`/`run` call touches `alive/<session8>` beside the run ledger, and a sweep leaves alone any session seen within `--live-window` (default 120 min). A session with no heartbeat counts as dead, so worktrees predating the mechanism stay reclaimable.
...
| `skills/run/references/isolation.md` | New in v0.14.0, reframed in v0.16.0 (English): §0 makes worktree isolation the default whenever the tree is dirty (the accident data, why prose prohibitions lose, and the `agent-exec isolate` lifecycle), followed by baseline/checkpoint mechanics and jj recovery, branch hygiene (disposable descendants, a clean PR branch built from the accepted diff), worktree-isolated parallelism and supervisor merge, competing implementations and how to read their divergence, verifying on a context-free integration tree, and front-loading approvals for non-interactive workers. Not loaded automatically. | — |
| `skills/run/references/config.md` | Detail split out of the playbook in v0.12.0 (English, verbatim): the four-layer config schema and deep-merge algorithm (plus its pre-v0.11.0 upgrade trap), `agent-exec route`/`dispatch` gating, external-executor dispatch mechanisms and the `dispatch: cli` sandboxing caveat, `enforcement.light_class`/`worker_vcs`/`worker_tree`/`turn_edits` and their escape hatches, and the full telemetry field allowlist and CLI. Not loaded automatically; read only when a configuration question actually arises. | — |
| `skills/cleanup/SKILL.md` | New in v0.30.0. Companion cleanup skill (`/cleanup`, or `orchestra:cleanup` cross-plugin). Drives `agent-exec isolate sweep` to reclaim leftover orchestra worktrees **repo-wide** — the ones `SessionEnd` never got to because the session crashed, predates the hook, ran with it disabled, or left a dirty tree behind on purpose. Reports a dry run first, keeps anything holding uncollected work, and offers `isolate diff`/`collect`/`--force` per dirty tree rather than deciding for the user. | — |
...
| `skills/run/references/external-executors.md` | Operational reference for external executors (Japanese) — Codex's Sol/Terra/Luna + effort policy and default `dispatch: cli` CLI/session-resume recipe (`dispatch: agent`/`codex:codex-rescue` as an override), Copilot's model catalog and CLI usage recipes (including session continuation), and official per-token pricing for Codex/Copilot/Claude. Not loaded automatically; read on demand when actually dispatching to Codex/Copilot. | — |
...
| `hooks/guard-worker-vcs.sh` | On by default (`enforcement.worker_vcs: "block"`). A `PreToolUse` guard denying destructive VCS commands (`checkout -- <paths>`, `checkout .`, `restore` without `--staged`, `reset --hard/--merge/--keep`, `clean -f`, `stash push`, `worktree remove --force`) when they come from a **subagent** and target a tree that is not one of orchestra's own (`orchestra/*` branch). Ships on, unlike `enforce-router.sh`, because it prevents data loss rather than steering a cost decision. The main thread is untouched, a worker inside its own worktree is untouched, `orchestra-delegate` (which owns snapshots and rollbacks) is exempt, and heredoc/quoted text that merely *mentions* the commands is not mistaken for running them. Escapes: `[orchestra:allow-vcs: reason]`, `ORCHESTRA_VCS_GUARD=off`, and fail-open on any uncertainty. | — |
| `hooks/guard-worker-tree.sh` | On by default (`enforcement.worker_tree: "block"`). A `PreToolUse` guard denying a **worker subagent that already has an orchestra worktree of its own** (a `git worktree list` branch under `orchestra/<session8>/`) from writing into the user's **main** working tree — the first worktree git lists. `Edit`/`Write`/`MultiEdit`/`NotebookEdit` are covered with zero parsing via `tool_input.file_path`/`notebook_path`; `Bash` is deliberately narrow (`> PATH`, `>> PATH`, `tee PATH`, `sed -i PATH`, after the same heredoc/quoted-string stripping) and `cp`/`mv`/`rm`/interpreter invocations are **not** analyzed on purpose. Paths are compared by components after `realpath`, so symlinks, `..` escapes, and the `/repo-worktrees/x` vs `/repo` prefix trap all resolve correctly. The main thread has no `agent_id` and is untouched, `orchestra-delegate` (which integrates worker output) is exempt, and a session with no orchestra worktree never triggers it. Escapes: `[orchestra:allow-tree: reason]`, `ORCHESTRA_TREE_GUARD=off`, and fail-open on any uncertainty. | — |
| `hooks/count-turn-edits.sh` | Turn-size tripwire (`enforcement.turn_edits`, default 8). A `PostToolUse` hook counting the **main thread's** edits within one turn; on crossing the threshold it injects one request to re-classify into the orchestrated lane. It never blocks an edit. Turn-level transcript analysis found orchestra invoked in 1 of the 102 turns that went on to hand-edit 10+ files: the prompts producing those turns ("作業を続けて", "実装して", "y") look small at classification time and only reveal their size once work is underway. Subagent edits are not counted — that is the delegation working. Silent in cheap-model sessions; `ORCHESTRA_TURN_GUARD=off` disables it. | — |
| `hooks/cleanup-worktrees.sh` | On by default (`enforcement.session_cleanup: "on"`). A `SessionEnd` hook that removes the finishing session's orchestra worktrees (`agent-exec isolate remove --session <id>`), keeping any that still hold uncollected changes — a dirty worktree is the only record of what a worker tried. As of v0.30.0 this is a **backstop**, not the mechanism: the pipeline sweeps at run end instead, since a killed or crashed session never reaches `SessionEnd` at all. An unrecognized config value falls back to `"on"`; `ORCHESTRA_SESSION_CLEANUP=off` disables it regardless of config. | — |
| `examples/orchestra.yaml` | Sample configuration file (YAML, fully commented). Copy to `.claude/orchestra.yaml` (project) or `~/.claude/orchestra.yaml` (user) to override model tiers, declare external executors (now `enabled: true` by default), or set `enforcement.light_class`/`worker_vcs`/`turn_edits`/`session_cleanup`, or use `/setup` to author it interactively. | — |
...
- **`external_executors`** lets non-Claude executors (Codex, Copilot, opencode, ...) participate as implementers and/or independent reviewers. `dispatch: "agent"` routes through an installed plugin subagent (`agentType`/`subagent_type`), falling back to normal model tiers when the name doesn't resolve — Codex's `codex:codex-rescue` supports this as an explicit override; `dispatch: "cli"` runs a non-interactive CLI via a cheap Haiku relay agent so the CLI's output never lands in the instructor's context, and this is now the *default* for Codex too (direct `codex exec`/`codex exec resume <SESSION_ID>`), alongside Copilot and opencode. Each executor's `class_policy` pins a concrete model+effort per class/role (Codex: Sol/Terra/Luna; Copilot: its own multi-provider catalog) — see `skills/run/references/config.md` for the benchmark-sourced policy and CLI usage/session-continuation recipes for all three `dispatch: cli` executors, and `skills/run/references/external-executors.md` §1 for why Codex moved off `dispatch: agent` by default. Every `dispatch: cli` executor with a session concept gets a per-task session store (`~/.claude/orchestra/sessions/<executor>-<task>.json`) that `agent-exec dispatch --task <id>` auto-resumes unless `--resume`/`--no-resume` says otherwise; `agent-exec dispatch session --task <id> [--executor N] [--json]` inspects it read-only. Because Copilot uses `dispatch: cli`, the relay's CLI call must be allowlisted in Claude Code's `settings.json`. The recommended path is the bundled `agent-exec` wrapper (installed once via `agent-exec install`), which needs only a single `Bash(agent-exec:*)` rule in `permissions.allow` — the current Copilot CLI runs autonomously in `-p` mode with just `--add-dir`, so no `COPILOT_ALLOW_ALL` env or `--allow-all-tools` flag is required (and Copilot's own `--allow-tool`/`--deny-tool` flags don't reliably confine it, since the agent reroutes around a removed tool — real containment needs an external sandbox). Without the wrapper, the manual alternative is a `Bash(copilot:*)` allow rule. `/setup` offers to configure this.
