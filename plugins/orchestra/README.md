...
So as of v0.15.0 the fix is structural. `agent-exec dispatch` isolates by default (`--isolate auto`) whenever the tree holds uncommitted work, giving the worker a tree that contains its own work and nothing else — the mistake becomes a no-op. This covers external CLI executors, which the Workflow tool's own `isolation: 'worktree'` option cannot reach even though that is where most implementation work runs.
