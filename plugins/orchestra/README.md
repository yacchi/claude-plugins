...
A second layer, `hooks/guard-worker-vcs.sh` (`enforcement.worker_vcs`, on by default), denies destructive VCS commands from *subagents* against any non-`orchestra/*` branch. The main thread is never affected, and inside its own worktree a worker may reset freely.
...
| `examples/orchestra.yaml` | Sample configuration file (YAML, fully commented). Copy to `.claude/orchestra.yaml` (project) or `~/.claude/orchestra.yaml` (user) to override model tiers, declare external executors (now `enabled: true` by default), or set `enforcement.light_class`/`worker_vcs`/`turn_edits`, or use `/setup` to author it interactively. | — |
